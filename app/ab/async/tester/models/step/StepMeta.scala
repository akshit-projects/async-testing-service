...
object StepMeta {
  ...
  implicit val decodeStepMeta: Decoder[StepMeta] = new Decoder[StepMeta] {
    def apply(c: HCursor): Decoder.Result[StepMeta] = {
      ...
      c.field("function").as[String].toOption match {
        case Some(metaType) => metaType match {
          case StepFunctions.HTTP_API_STEP => c.as[HttpStepMeta]
          case StepFunctions.SUBSCRIBE_KAFKA_MESSAGES_STEP => c.as[KafkaSubscribeMeta]
          case StepFunctions.PUBLISH_KAFKA_MESSAGE_STEP => c.as[KafkaPublishMeta]
          case StepFunctions.DELAY_STEP => c.as[DelayStepMeta]
          case unknown => Left(DecodingFailure(s"Unknown meta type: $unknown", c.history))
        }
        case None => 
          // No explicit type field, try to infer from fields
