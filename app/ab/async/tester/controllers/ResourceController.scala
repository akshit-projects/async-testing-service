...
class ResourceController @Inject() (
  ...
  def getResources: Action[AnyContent] =
    authorizedAction.requirePermission(Permissions.RESOURCES_READ).async {
      implicit request =>
        MetricUtils.withAPIMetrics("getResources") {
          val typesOpt =
            request.getQueryString("types").map(_.split(",").toList)
          ...
          resourceService
            .getResources(typesOpt, groupOpt, namespaceOpt, limit, page)
            .map { paginatedRes =>
              Ok(paginatedRes.asJson.noSpaces)
            } recover { case ex =>
            logger.error("getResources failed", ex)
            InternalServerError(
              Map("error" -> "failed to list resources").asJsonNoSpaces
            )
          }
        }
    }

  def getResource(id: String): Action[AnyContent] =
    authorizedAction.requirePermission(Permissions.RESOURCES_READ).async {
      implicit request =>
        MetricUtils.withAPIMetrics("getResource") {
          resourceService.getResourceById(id).map {
            ...
            case None           =>
              NotFound(
                Map("error" -> s"resource not found: $id").asJsonNoSpaces
              )
          } recover { case ex =>
            logger.error(s"getResource $id failed", ex)
            InternalServerError(
              Map("error" -> "failed to fetch resource").asJsonNoSpaces
            )
          }
        }
    }
  ...
  def createResource: Action[AnyContent] =
    authorizedAction.requirePermission(Permissions.RESOURCES_CREATE).async {
      implicit request =>
        MetricUtils.withAPIMetrics("createResource") {
          JsonParsers
            .parseJsonBody[ResourceConfig](request)(implicitly, ec) match {
            ...
            case Right(conf)  =>
              resourceService.createResource(conf).map { created =>
                ...
                InternalServerError(
                  Map("error" -> "failed to create resource").asJsonNoSpaces
                )
              }
          }
        }
  ...
  def updateResource(): Action[AnyContent] =
    authorizedAction.requirePermission(Permissions.RESOURCES_UPDATE).async {
      implicit request =>
            ...
            case Right(conf)  =>
              resourceService.updateResource(conf).map {
                ...
                case None          =>
                  NotFound(
                    Map(
                      "error" -> s"resource not found: ${conf.getId}"
                    ).asJsonNoSpaces
                  )
              } recover { case ex =>
                logger.error("updateResource failed", ex)
                InternalServerError(
                  Map("error" -> "failed to update resource").asJsonNoSpaces
                )
              }
          }
        }
  ...
  def deleteResource(id: String): Action[AnyContent] =
    authorizedAction.requirePermission(Permissions.RESOURCES_DELETE).async {
      implicit request =>
        MetricUtils.withAPIMetrics("deleteResource") {
          resourceService.deleteResource(id).map { deleted =>
            ...
            InternalServerError(
              Map("error" -> "failed to delete resource").asJsonNoSpaces
            )
          }
        }
    }
