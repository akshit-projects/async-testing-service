...
class AuthController @Inject() (
    ...
    MetricUtils.withAPIMetrics("loginWithEmail") {
      JsonParsers.parseJsonBody[EmailLoginRequest](request) match {
        ...
        case Right(loginRequest) =>
          authService
            .loginWithEmail(loginRequest.email, loginRequest.password)
            .map { authResponse =>
              Ok(authResponse.asJson.noSpaces).as("application/json")
            }
            .recover {
              case e: ValidationException =>
                ...
                Unauthorized(GenericError(e.getMessage).asJson.noSpaces)
                  .as("application/json")
              case e: Exception =>
                logger.error(s"Login error: ${e.getMessage}", e)
                InternalServerError(
                  GenericError("Login failed").asJson.noSpaces
                ).as("application/json")
            }
      }
    }
          ...
          authService
            ...
            .recover {
              ...
              case e: Exception =>
                ...
                InternalServerError(
                  GenericError("User registration failed").asJson.noSpaces
                ).as("application/json")
            }
      }
    }
    ...
    MetricUtils.withAPIMetrics("forgotPassword") {
      JsonParsers.parseJsonBody[ForgotPasswordRequest](request) match {
        ...
        case Right(forgotRequest) =>
          authService
            .forgotPassword(forgotRequest.email)
            .map { success =>
              if (success) {
                Ok(
                  Map(
    ...
    MetricUtils.withAPIMetrics("resetPassword") {
      JsonParsers.parseJsonBody[ResetPasswordRequest](request) match {
        ...
        case Right(resetRequest) =>
          authService
            .resetPassword(resetRequest.token, resetRequest.newPassword)
            .map { success =>
              if (success) {
                Ok(
                  Map("message" -> "Password reset successful").asJson.noSpaces
                ).as("application/json")
              } else {
                InternalServerError(
                  GenericError("Failed to reset password").asJson.noSpaces
                ).as("application/json")
              }
            }
            .recover {
  ...
  def refreshAccessToken(): Action[AnyContent] = Action.async {
    implicit request =>
      MetricUtils.withAPIMetrics("refreshAccessToken") {
        request.body.asJson.flatMap(x =>
          x.asJson.hcursor.get[String]("refreshToken").toOption
        ) match {
            ...
            authService
              ...
              .recover {
                ...
                case e: Exception =>
                  ...
                  InternalServerError(
                    GenericError("Token refresh failed").asJson.noSpaces
                  ).as("application/json")
              }
          case None =>
            Future.successful(
              BadRequest(GenericError("Refresh token required").asJson.noSpaces)
                .as("application/json")
            )
        }
      }
