...
/**
 * Unified controller for user management and profile operations
 * Combines admin user management and authenticated user profile endpoints
 */
@Singleton
class UserManagementController @Inject()(
   ...
   * GET /api/v1/profile
   */
  def getCurrentUserProfile: Action[AnyContent] = authenticatedAction.async { implicit request =>
   ...
   * PUT /api/v1/profile
   */
  def updateCurrentUserProfile(): Action[AnyContent] = authenticatedAction.async { implicit request =>
