        ...
        authenticatedAction.invokeBlock(
          ...
          (authRequest: AuthenticatedRequest[A]) =>
            if (hasPermission(authRequest.user, permission)) {
              ...
              Future.successful(
                Results.Forbidden(
                  GenericError("Insufficient permissions").asJson.noSpaces
                )
              )
            }
        )
    ...
    new ActionBuilder[AuthenticatedRequest, AnyContent] {
      ...
      override def invokeBlock[A](
        ...
        authenticatedAction.invokeBlock(
          ...
          (authRequest: AuthenticatedRequest[A]) =>
            if (
              authRequest.user.isAdmin || authRequest.user.role == UserRole.Admin
            ) {
              block(authRequest)
            } else {
              Future.successful(
                Results.Forbidden(
                  GenericError("Admin access required").asJson.noSpaces
                )
              )
            }
        )
