...
class ExecutionController @Inject()(
  ...
  def getExecution(executionId: String): Action[AnyContent] = authorizedAction.requirePermission(Permissions.EXECUTIONS_READ).async { implicit request =>
  ...
  def getExecutions(): Action[AnyContent] = authorizedAction.requirePermission(Permissions.EXECUTIONS_READ).async { implicit request =>
  ...
  private def handleExecutionStream(executionId: String, clientId: Option[String]): Flow[String, String, NotUsed] = {
    ...
    val executionUpdates = executionsService.streamExecutionUpdates(executionId, clientId)
