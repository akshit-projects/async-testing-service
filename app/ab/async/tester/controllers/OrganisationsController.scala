...
class OrganisationsController @Inject()(
  ...
  def createOrganisation(): Action[AnyContent] = authorizedAction.requirePermission(Permissions.ORG_CREATE).async { implicit request =>
    JsonParsers.parseJsonBody[Organisation](request)(implicitly, ec) match {
      case Left(result) => Future.successful(result)
      case Right(organisation) =>
        organisationService.createOrganisation(organisation).map { created =>
  ...
  def getOrganisation(id: String): Action[AnyContent] = authorizedAction.requirePermission(Permissions.ORG_READ).async { implicit request =>
    organisationService.getOrganisation(id).map {
      case Some(organisation) =>
        Ok(organisation.asJson.noSpaces).as("application/json")
      case None =>
        NotFound(Map("error" -> "Organisation not found").asJson.noSpaces).as("application/json")
    }
  }

  /** GET /v1/organisations - get all organisations with optional search */
  def getOrganisations(search: Option[String], limit: Option[Int], page: Option[Int]): Action[AnyContent] = 
    authorizedAction.requirePermission(Permissions.ORG_READ).async { implicit request =>
      val actualLimit = limit.getOrElse(50).min(100)
      val actualPage = page.getOrElse(0).max(0)
      
      organisationService.getOrganisations(search, actualLimit, actualPage).map { paginatedOrganisations =>
        Ok(paginatedOrganisations.asJson.noSpaces).as("application/json")
      }
    }

  /** PUT /v1/organisations/:id - update organisation */
  def updateOrganisation(id: String): Action[AnyContent] = authorizedAction.requirePermission(Permissions.ORG_UPDATE).async { implicit request =>
    JsonParsers.parseJsonBody[Organisation](request)(implicitly, ec) match {
      case Left(result) => Future.successful(result)
      case Right(organisation) =>
        val orgWithId = organisation.copy(id = Some(id))
        organisationService.updateOrganisation(orgWithId).map { updated =>
          if (updated) {
  ...
  def deleteOrganisation(id: String): Action[AnyContent] = authorizedAction.requirePermission(Permissions.ORG_DELETE).async { implicit request =>
