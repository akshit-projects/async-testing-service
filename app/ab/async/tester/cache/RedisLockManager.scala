 ...
 */
@Singleton
class RedisLockManager @Inject()(configuration: Configuration)(implicit ec: ExecutionContext) {
   ...
   */
  def createLockKey(namespace: String, keys: String*): String = {
    if (keys.isEmpty) {
   ...
   */
  def releaseLock(lockKey: String): Boolean = {
    withJedis { jedis =>
  ...
  /**
   * Convenience method to release a lock for Kafka consumers
   */
  def releaseKafkaConsumerLock(groupId: String, topicName: String): Boolean = {
    val lockKey = createKafkaConsumerLockKey(groupId, topicName)
    releaseLock(lockKey)
  }
