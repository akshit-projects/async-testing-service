...
import ab.async.tester.models.execution.{ExecutionStatus, ExecutionStatusUpdate, StatusUpdateType}
import ab.async.tester.models.flow.Floww
import ab.async.tester.models.step.{FlowStep, StepError, StepResponse}
import ab.async.tester.repository.execution.ExecutionRepository
...
class FlowRunnerImpl @Inject()(
  ...
  private def executeFlow(flow: Floww, statusChannel: ActorRef): Future[Unit] = {
    ...
    executionRepository.saveExecution(flow).flatMap { execution =>
      ...
      executionRepository.updateStatus(executionId, ExecutionStatus.InProgress)
  ...
  private def executeFlowSteps(steps: List[FlowStep], statusChannel: ActorRef): Future[Boolean] = {
    ...
    def executeStep(step: FlowStep): Future[StepResponse] = {
      ...
      val runner = stepRunnerRegistry.getRunnerForStep(step.function)
