...
import ab.async.tester.cache.{KafkaResourceCache, RedisLockManager}
import ab.async.tester.models.enums.StepStatus
import ab.async.tester.models.resource.KafkaConfig
import ab.async.tester.models.step._
    ...
    resourceRepository.findById(resourceId).map { resource =>
      try {
        val kafkaConfig = resource match {
          case Some(kafkaConfig: KafkaConfig) => kafkaConfig
          case None =>
        ...
        val producer = kafkaResourceCache.getOrCreateProducer(resourceId, new KafkaProducer[String, String](props))
...
class KafkaConsumerStepRunner @Inject()(
  ...
  override protected def executeStep(step: FlowStep, previousResults: List[StepResponse]): Future[StepResponse] = {
    ...
    resourceRepository.findById(resourceId).flatMap { resource =>
      ...
      if (step.runInBackground) {
        ...
        redisLockManager.withKafkaConsumerLock(kafkaMeta.groupId, kafkaMeta.topicName, step.timeout, step.timeout / RedisLockManager.DefaultRetryDelay) {
  ...
  private def startBackgroundConsumer(step: FlowStep, kafkaMeta: KafkaSubscribeMeta, kafkaConfig: KafkaConfig): Future[StepResponse] = {
      ...
      val consumerThread = new Thread(() => {
        ...
        try {
          while (!Thread.currentThread().isInterrupted) {
            ...
            if (System.currentTimeMillis() - startTime > step.timeout && !promise.isCompleted) {
  ...
  private def regularConsumer(step: FlowStep, kafkaMeta: KafkaSubscribeMeta, kafkaConfig: KafkaConfig): Future[StepResponse] = {
    val stepId = step.id.getOrElse(UUID.randomUUID().toString)
  ...
  private def createConsumerProps(kafkaMeta: KafkaSubscribeMeta, kafkaConfig: KafkaConfig): Properties = {
    val props = new Properties()
