...
class ResourceServiceImpl @Inject() (
  ...
  /** Get all resources, optionally filtered by request parameters
    *
    * @param request
    *   the filter criteria for resources
    * @return
    *   list of resource configs
    */
  override def getResources(
      typesOpt: Option[List[String]],
      groupOpt: Option[String],
      namespaceOpt: Option[String],
      limit: Int,
      page: Int
  ): Future[PaginatedResponse[ResourceConfig]] = {
    MetricUtils.withAsyncServiceMetrics(serviceName, "getResources") {
      resourceRepository
        .findAllWithCount(typesOpt, groupOpt, namespaceOpt, limit, page)
        .map { case (resources, total) =>
          PaginatedResponse(
            data = resources,
        ...
        .recover { case e: Exception =>
          logger.error(
            s"Error retrieving resources with pagination: ${e.getMessage}",
            e
          )
  ...
  override def getResourceById(id: String): Future[Option[ResourceConfig]] = {
    MetricUtils.withAsyncServiceMetrics(serviceName, "getResourceByID") {
      resourceRepository.findById(id).recover { case e: Exception =>
  ...
  /** Create a new resource
    ...
    * @return
    *   the created resource with ID
    */
  override def createResource(
      resourceConfig: ResourceConfig
  ): Future[ResourceConfig] = {
    MetricUtils.withAsyncServiceMetrics(serviceName, "createResource") {
      resourceRepository.create(resourceConfig).recover { case e: Exception =>
  ...
  override def updateResource(
      resourceConfig: ResourceConfig
  ): Future[Option[ResourceConfig]] = {
    MetricUtils.withAsyncServiceMetrics(serviceName, "updateResource") {
      resourceRepository.update(resourceConfig).recover { case e: Exception =>
        dbMetrics.labels("update", "resources", "error").inc()
        logger.error(
          s"Error updating resource ${resourceConfig.getId}: ${e.getMessage}",
          e
        )
        None
      }
  ...
  override def deleteResource(id: String): Future[Boolean] = {
    MetricUtils.withAsyncServiceMetrics(serviceName, "deleteResource") {
      resourceRepository.delete(id).recover { case e: Exception =>
