...
trait UserManagementService {
  def getAllUsers(
      ...
      limit: Int,
      page: Int
  ): Future[PaginatedResponse[DetailedUserProfile]]
  def getUserById(userId: String): Future[Option[DetailedUserProfile]]
  def updateUser(
      userId: String,
      updateRequest: UpdateUserRequest
  ): Future[DetailedUserProfile]
}
...
class UserManagementServiceImpl @Inject() (
  ...
  override def getAllUsers(
    ...
    MetricUtils.withAsyncServiceMetrics(serviceName, "getAllUsers") {
      ...
      userProfileRepository
        .findAllWithAuth(search, limit, page)
        .map { case (profilesWithAuth, total) =>
  ...
  override def getUserById(
      userId: String
  ): Future[Option[DetailedUserProfile]] = {
            ...
            try {
              ...
              case _: Exception =>
                throw ValidationException(
                  s"Invalid role: $roleStr. Valid roles are: admin, user, viewer"
                )
            }
          }
                ...
                getUserById(userId).map {
                  ...
                  case None =>
                    throw new IllegalStateException(
                      s"User $userId not found after update"
                    )
                }
              } else {
                throw new IllegalStateException(
                  s"Failed to update user $userId"
                )
              }
            }
