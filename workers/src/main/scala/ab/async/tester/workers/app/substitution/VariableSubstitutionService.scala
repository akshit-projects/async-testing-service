...
class VariableSubstitutionService @Inject()() {
   ...
   * @param previousResults Map of step name to step response for variable resolution
   * @return ExecutionStep with variables substituted in meta
   */
  def substituteVariablesInStep(step: ExecutionStep, previousResults: List[StepResponse]): ExecutionStep = {
    // Convert previous results to a map by step name for easier lookup
    val stepResponseMap = previousResults.map(r => r.name -> r).toMap
    
    try {
      val substitutedMeta = substituteVariablesInStepMeta(step.meta, stepResponseMap)
      step.copy(meta = substitutedMeta)
    } catch {
      case e: Exception =>
        logger.error(s"Error substituting variables in step ${step.name}: ${e.getMessage}", e)
        throw new RuntimeException(s"Variable substitution failed for step ${step.name}: ${e.getMessage}", e)
    }
  }
   ...
   */
  private def substituteVariablesInStepMeta(meta: StepMeta, stepResponses: Map[String, StepResponse]): StepMeta = {
    meta match {
   ...
   */
  private def substituteVariablesInHttpMeta(meta: HttpStepMeta, stepResponses: Map[String, StepResponse]): HttpStepMeta = {
    meta.copy(
      body = meta.body.map(VariableSubstitution.substituteVariables(_, stepResponses)),
      headers = meta.headers.map(_.map { case (key, value) =>
        key -> VariableSubstitution.substituteVariables(value, stepResponses)
      }),
      expectedStatus = meta.expectedStatus.map(VariableSubstitution.substituteVariables(_, stepResponses)),
      expectedResponse = meta.expectedResponse.map(VariableSubstitution.substituteVariables(_, stepResponses))
    )
  }
   ...
   */
  private def substituteVariablesInKafkaSubscribeMeta(meta: KafkaSubscribeMeta, stepResponses: Map[String, StepResponse]): KafkaSubscribeMeta = {
    meta.copy(
      topicName = VariableSubstitution.substituteVariables(meta.topicName, stepResponses),
      groupId = VariableSubstitution.substituteVariables(meta.groupId, stepResponses)
    )
  }
   ...
   */
  private def substituteVariablesInKafkaPublishMeta(meta: KafkaPublishMeta, stepResponses: Map[String, StepResponse]): KafkaPublishMeta = {
    meta.copy(
      topicName = VariableSubstitution.substituteVariables(meta.topicName, stepResponses),
      messages = meta.messages.map { msg =>
        msg.copy(
          key = msg.key.map(VariableSubstitution.substituteVariables(_, stepResponses)),
