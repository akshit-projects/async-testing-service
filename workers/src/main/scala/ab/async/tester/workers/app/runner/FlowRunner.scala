...
class FlowRunnerImpl @Inject() (
  ...
  override def startFlowConsumer(): Unit = {
    ...
    KafkaConsumer
      .subscribeTopic(workerQueueTopic)
      .mapAsync(10) { msg =>
        ...
        decode[Execution](messageValue) match {
          case Right(execution) =>
            ...
            val initialDelay =
              getInitialDelay(execution.parameters).getOrElse(0)
            after(initialDelay.milliseconds, using = system.scheduler) {
              logger.info(
                s"Starting execution ${execution.id} flow after $initialDelay"
              )
              ...
              logger.error(
                s"Failed to process execution ${execution.id}: ${e.getMessage}",
                e
              )
          ...
          case Left(error) =>
            logger.error(
              s"Failed to parse execution from Kafka message: $error"
            )
  ...
  override def executeFlow(execution: Execution): Future[Unit] = {
    MetricUtils.withAsyncServiceMetrics(runnerName, "executeFlow") {
      logger.info(
        s"Starting execution: ${execution.id} with ${execution.variables.length} variables"
      )
      ...
      updateStatus(execution, ExecutionStatus.InProgress).flatMap { _ =>
        ...
        executeFlowSteps(execution.id, execution.steps, execution.variables)
          .map { successful =>
            val finalStatus =
              if (successful) ExecutionStatus.Completed
              else ExecutionStatus.Failed
            val message =
              if (successful) "Execution completed successfully"
          ...
          .recoverWith { case e: Exception =>
            logger.error(
              s"Execution ${execution.id} failed with exception: ${e.getMessage}",
              e
            )
            publishExecutionUpdate(
              execution.id,
  ...
  private def updateStatus(execution: Execution, status: ExecutionStatus) = {
    ...
    val updateExecutionStatus =
      executionRepository.updateStatus(execution.id, status, isCompleted)
    val updateTestSuiteExecutionStatus = execution.testSuiteExecutionId match {
      case Some(testSuiteExecutionId) =>
        testSuiteExecutionRepository.updateTestSuiteExecution(
    ...
    for {
      ...
      logger.info(
        s"Updated status for execution ${execution.id} with status $status"
      )
    }
  }
  ...
  private def executeFlowSteps(
    ...
    def executeStep(step: ExecutionStep): Future[StepResponse] = {
      logger.info(
        s"Executing step ${step.name} (background: ${step.runInBackground})"
      )
      ...
      val steps = if (variables.nonEmpty) {
        ...
        val substitutedSteps = RuntimeVariableSubstitution
          .substituteVariablesInSteps(List(flowStep), variables)
      ...
      executionRepository
        .updateExecutionStep(executionId, stepId, updatedStep)
        .flatMap { _ =>
          publishStepUpdate(
            executionId,
            stepId,
    ...
    // Execute steps sequentially
    def processStepsSequentially(
        remainingSteps: List[ExecutionStep]
    ): Future[Boolean] = {
      if (remainingSteps.isEmpty) {
        ...
        if (backgroundSteps.isEmpty) {
          ...
          logger.info(
            s"Waiting for ${backgroundSteps.size} background steps to complete"
          )
        ...
        if (currentStep.runInBackground) {
            ...
            case Success(response) =>
              ...
              executionRepository
                ...
                .map { _ =>
                  ...
                  if (
                    response.status == StepStatus.ERROR && !currentStep.continueOnSuccess
                  ) {
                    logger.error(
                      s"Background step ${currentStep.name} failed with error status"
                    )
                  }
                }
            case Failure(e) =>
              logger.error(
                s"Background step ${currentStep.name} failed: ${e.getMessage}",
                e
              )
              ...
              val updatedStep = currentStep.copy(
                ...
                response = Some(errorResponse.response)
              )
              executionRepository
                .updateExecutionStep(
                  executionId,
          ...
          executeStep(currentStep)
              ...
              executionRepository
                ...
                .flatMap { _ =>
                  ...
                  if (
                    ...
                    logger.error(
                      s"Step ${currentStep.name} failed with error status, stopping flow"
                    )
                    Future.successful(false)
                  } else {
                    // Check if this was a Condition step that generated new steps
                    val nextStepsToRun = response.response match {
                      case conditionResponse: ConditionResponse =>
                        if (conditionResponse.generatedSteps.nonEmpty) {
                          logger.info(
                            s"Injecting ${conditionResponse.generatedSteps.size} steps from condition ${currentStep.name}"
                          )
                          // Convert FlowSteps to ExecutionSteps
                          // We need unique IDs for them.
                          val newExecutionSteps =
                            conditionResponse.generatedSteps.map { flowStep =>
                              val executionStepId =
                                java.util.UUID.randomUUID().toString
                              // Ensure status is initialized
                              ExecutionStep(
                                id = Some(executionStepId),
                                name = flowStep.name,
                          ...
                          // Prepend new steps to the existing next steps
                          newExecutionSteps ++ nextSteps
                        } else {
                          nextSteps
                        }
                      case _ => nextSteps
                    ...
                    processStepsSequentially(nextStepsToRun)
                  }
                }
            ...
            .recoverWith { case e: Exception =>
              logger.error(
                s"Step ${currentStep.name} failed: ${e.getMessage}",
                e
              )
              ...
              executionRepository
                ...
                .flatMap { _ =>
                  ...
                  if (!currentStep.continueOnSuccess) {
                    ...
                    logger.warn(
                      s"Step ${currentStep.name} failed but continueOnSuccess=true, continuing flow"
                    )
                    processStepsSequentially(nextSteps)
                  }
                }
  ...
  /** Convert ExecutionStep to FlowStep for processing
    */
  private def convertExecutionStepToFlowStep(
  ...
  /** Publish execution status update to Redis
    */
  private def publishExecutionUpdate(
      executionId: String,
      message: String,
    ...
    Try {
      ...
      try {
        ...
        logger.info(
          s"Published execution update for $executionId: $message and result $result"
        )
      } finally {
        jedis.close()
      }
      ...
      logger.error(
        s"Failed to publish execution update for $executionId: ${e.getMessage}",
        e
      )
    }
  }
  ...
  /** Publish step status update to Redis
    */
  private def publishStepUpdate(
      executionId: String,
      stepId: String,
      message: String,
      status: StepStatus,
      stepResponse: Option[StepResponseValue] = None
  ): Unit = {
    Try {
      ...
      val update = ExecutionStatusUpdate(
        executionId = executionId,
        updateType =
          ab.async.tester.domain.enums.ExecutionUpdateType.STEP_UPDATE,
      ...
      try {
        ...
        logger.debug(
          s"Published step update for $executionId/$stepId: $message: $status"
        )
      } finally {
        jedis.close()
      }
