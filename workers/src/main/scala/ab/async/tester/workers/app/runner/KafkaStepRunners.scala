    ...
    val kafkaMeta = step.meta match {
      ...
      case _                      =>
        throw new IllegalStateException(
          s"Invalid step meta found for publishing kafka message for step ${step.name}"
        )
    }
    ...
    resourceRepository.findById(resourceId).map { resource =>
      try {
        val kafkaConfig = resource match {
          ...
          case None                                   =>
            throw new IllegalStateException(
              s"The resource for step: ${step.name} is not found"
            )
          case _ =>
            throw new IllegalStateException(
              s"The resourceId is not valid for step: ${step.name}"
            )
        }
        if (kafkaConfig.brokersList.isEmpty) {
          throw new IllegalArgumentException(
            "Bootstrap servers are required for Kafka publisher"
          )
        }

        if (kafkaMeta.topicName.isEmpty) {
          throw new IllegalArgumentException(
            "Topic name is required for Kafka publisher"
          )
        }

        if (kafkaMeta.messages.isEmpty) {
          throw new IllegalArgumentException(
            "At least one message is required for Kafka publisher"
          )
        }
        ...
        val producer = kafkaResourceCache.getOrCreateProducer(
          resourceId,
          kafkaClient.getKafkaPublisher(kc)
        )
        ...
        try {
          ...
          case e: Exception =>
            logger.error(
              s"Error in Kafka publisher step ${step.name}: ${e.getMessage}",
              e
            )
...
class KafkaConsumerStepRunner @Inject() (
  ...
  /** Check if a Kafka message matches the search pattern Supports exact string
    * matching and JSONPath queries
    */
  private def matchesSearchPattern(
      message: KafkaMessage,
      pattern: Option[KafkaSearchPattern]
  ): Boolean = {
    pattern match {
      case None => true // No filter, accept all messages

      case Some(sp) if sp.matchType == "exact" =>
        // Exact string match (case-sensitive contains)
        message.value.contains(sp.pattern)

      case Some(sp) if sp.matchType == "jsonpath" =>
        // JSONPath query support
        parse(message.value).fold(
          _ => false, // Not valid JSON, skip
          json => {
            // Parse JSONPath pattern (simplified - supports dot notation like "$.data.userId")
            val path = sp.pattern
              .stripPrefix("$")
              .stripPrefix(".")
              .split("\\.")
            ...
            path.foreach { key =>
              current.hcursor.downField(key).focus match {
            ...
            found && !current.isNull
          }
        )

      case _ => true // Unknown match type, accept all
    }
  }
  ...
  override protected def executeStep(
    ...
    val kafkaMeta = step.meta match {
      ...
      case _                        =>
        throw new IllegalStateException(
          s"Invalid step meta found for subscribing kafka message for step ${step.name}"
        )
    }
    ...
    resourceRepository.findById(resourceId).flatMap { resource =>
      ...
      if (kafkaConfig.brokersList.isEmpty) {
        return Future.failed(
          new IllegalArgumentException(
            "Bootstrap servers are required for Kafka consumer"
          )
        )
      }
      ...
      if (kafkaMeta.topicName.isEmpty) {
        return Future.failed(
          new IllegalArgumentException(
            "Topic name is required for Kafka consumer"
          )
        )
      }
      ...
      if (step.runInBackground) {
        ...
        redisLockManager.withKafkaConsumerLock(
          kafkaMeta.groupId,
          kafkaMeta.topicName,
          step.timeoutMs,
          step.timeoutMs / RedisLockManager.DefaultRetryDelay
        ) {
  ...
  private def startBackgroundConsumer(
      step: ExecutionStep,
      kafkaMeta: KafkaSubscribeMeta,
      kafkaConfig: KafkaResourceConfig
  ): Future[StepResponse] = {
    logger.info(
      s"Starting background Kafka consumer for step ${step.name} on topic ${kafkaMeta.topicName}"
    )
      ...
      val consumerThread = new Thread(() => {
        ...
        try {
          while (!Thread.currentThread().isInterrupted) {
            ...
            for (record <- records.asScala) {
              ...
              if (messages.size % 10 == 0) {
                logger.debug(
                  s"Background Kafka consumer for step ${step.name} has received ${messages.size} messages so far"
                )
              }

              // If we reach max messages, we stop (if specified)
              if (
                kafkaMeta.maxMessages > 0 && messages.size >= kafkaMeta.maxMessages
              ) {
                logger.info(
                  s"Background Kafka consumer for step ${step.name} reached max messages: ${messages.size}"
                )
            ...
            if (
              System
                .currentTimeMillis() - startTime > step.timeoutMs && !promise.isCompleted
            ) {
              logger.info(
                s"Background Kafka consumer for step ${step.name} reached timeout with messages count: ${messages.size}"
              )
              val response = StepResponse(
                name = step.name,
                id = stepId,
          ...
          case e: InterruptedException =>
            logger.info(
              s"Background Kafka consumer for step ${step.name} was interrupted"
            )
          case e: Exception =>
            logger.error(
              s"Error in background Kafka consumer for step ${step.name}: ${e.getMessage}",
              e
            )
          ...
          synchronized {
            ...
            logger.info(
              s"Released lock for background consumer group ${kafkaMeta.groupId} on topic ${kafkaMeta.topicName}"
            )
          }

          logger.info(
            s"Background Kafka consumer for step ${step.name} has stopped after processing ${messages.size} messages"
          )
        }
      })
  ...
  private def regularConsumer(
    ...
    Future {
      ...
      try {
        ...
        while (
          messages.size < kafkaMeta.maxMessages &&
  ...
  private def createConsumerProps(
      kafkaMeta: KafkaSubscribeMeta,
      kafkaConfig: KafkaResourceConfig
  ): Properties = {
    ...
    props.put(
      ...
      if (kafkaMeta.fromBeginning) "earliest" else "latest"
    )
  ...
  def stopBackgroundConsumer(stepId: String): Unit = {
    synchronized {
      backgroundThreads.get(stepId).foreach { thread =>
        logger.info(
          s"Stopping background Kafka consumer thread for step ID: $stepId"
        )
        thread.interrupt()
      }
    }
